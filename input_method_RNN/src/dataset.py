...
class InputMethodDataset(Dataset):
    def __init__(self, path):
        ...
        # 通过 pandas 读取 JSONL 格式的文件，并将其转换为字典列表（每个字典包含 input 和 target 键值对），存储在 self.data 中
    def __len__(self):
        return len(self.data)

    def __getitem__(self, index):
        input_tensor = torch.tensor(self.data[index]['input'], dtype=torch.long)  # 将输入转换成tensor张量
