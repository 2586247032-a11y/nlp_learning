...
def predict_1batch(model,inputs,en_tokenizer):
    ...
    with torch.no_grad():
        ...
        # 隐藏状态
        decoder_hidden = context_vector.unsqueeze(0)  #unsqueeze(dim) 的作用是在指定位置增加一个维度
...
def run_predict():
    device, zh_tokenizer,en_tokenize, model = ready()
    ...
    while True:
        ...
        result = predict(user_input, device, zh_tokenizer,en_tokenize, model)
        print("英文：",result)
