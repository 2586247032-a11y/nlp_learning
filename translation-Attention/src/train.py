...
def train_one_epoch(model, dataloader, loss_fn, optimizer, device):
    ...
    for inputs, targets in tqdm(dataloader,desc = '训练'):
        ...
        decoder_targets = targets[:,1:]   # targets_output.shape = [batch_size, seq_len]
        # 前向传播
        # 编码阶段
        encoder_outputs, contex_vector = model.encoder(encoder_inputs)
        # contex_vector.shape = [batch_size, hidden_size]

        # 解码阶段
        decoder_hidden_0 = contex_vector.unsqueeze(0)
        # decoder_hidden_0.shape = [1, batch_size, hidden_size]

        decoder_outputs = []
        # 储存每一个时间步的预测结果，把整个预测结果去和decoder_targets做对比

        seq_len = decoder_inputs.shape[1]
        for i in range(seq_len):
            decoder_input = decoder_inputs[:,i].unsqueeze(1) # decoder_input.shape = [batch_size, 1]
            decoder_output, decoder_hidden_n = model.decoder(decoder_input, decoder_hidden_0, encoder_outputs)
            decoder_hidden_0 = decoder_hidden_n
            # decoder_output.shape = [batch_size, 1, vocab_size]
            decoder_outputs.append(decoder_output)

        # 目的：decoder_outputs:[tensor([batch_size,1,vocab_size])] -> [batch_size * seq_len, vocab_size]
