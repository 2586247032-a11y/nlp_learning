...
def process():
    print("开始处理数据")
    # 读取文件
    df=pd.read_csv(config.RAW_DATA_DIR / 'cmn.txt',sep='\t',header=None,usecols=[0,1],names=["en","zh"],encoding="utf-8").dropna()
    # sep='\t'指定分隔符    header=None第一行不是标题     usecols=[0,1]指定前两列索引     names=["en","zh"]指定列名
